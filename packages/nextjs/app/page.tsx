...
const Home = () => {
  ...
  const formattedBalance = contractBalance ? (Number(contractBalance) / (10 ** 18)).toFixed(6) : "0.0000000";
            ...
            <div className="p-4 bg-base-200 rounded-xl">
              ...
              <div className="bg-base-300 p-4 rounded-lg my-4">
                <p className="text-xl font-medium text-center">
                  Prize Pool: {formattedBalance?.toString()} STRK
                </p>
              </div>
              ...
              {events && events.length > 0 ? (
                events.map((event, index) => (
                  <div key={index} className="p-4 bg-base-200 rounded-xl">
                    <p className="text-lg">
                  <span className="font-medium">{event.parsedArgs.account.substring(0,6)}...{event.parsedArgs.account.substring(event.parsedArgs.account.lenght -4)}</span> incremented the counter
                    </p>
                    </div>
                ))
              ): (
